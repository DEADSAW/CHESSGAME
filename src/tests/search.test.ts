...
import { Color, DifficultyLevel, PlayStyle, type SearchOptions } from '../types';
...
const withDepth = (maxDepth: number): SearchOptions => ({ ...baseOptions, maxDepth });
const withTime = (maxTimeMs: number): SearchOptions => ({ ...baseOptions, maxTimeMs });

describe('Search', () => {
  describe('Basic Search', () => {
    it('should find a legal move from starting position', () => {
      const position = parseFen(STARTING_FEN);
        const result = search(position, withDepth(2));
    ...
    it('should find mate in 1', () => {
      ...
      const position = parseFen('r1bqkb1r/pppp1ppp/2n2n2/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 4 4');
        const result = search(position, withDepth(3));
  ...
  describe('Search Options', () => {
    it('should respect max depth', () => {
      const position = parseFen(STARTING_FEN);
        const result = search(position, withDepth(1));
    ...
    it('should respect time limit', () => {
      ...
      const startTime = Date.now();
        const result = search(position, withTime(100));
